...
// Iniciar la animación después de un breve retraso
            setTimeout(() => {
                ...
                setTimeout(() => {
                    ...
                    mensajeExito.innerHTML = `
                        ...
                        <p>Tu mensaje ha sido entregado por nuestra mascota. Pronto recibirás una respuesta.</p>
                    `;
        ...
        document.getElementById('formulario-contacto').addEventListener('submit', function(e) {
            ...
            burbuja.textContent = '¡Esta muy Bien, Voy a entregar tu mensaje ahora a esos mazapane que no programan bien!';
            burbuja.classList.add('visible');
        ...
        // Manejar la carga de archivos para mostrar el nombre del archivo
        document.getElementById('archivo').addEventListener('change', function() {
            const fileName = this.files[0] ? this.files[0].name : 'Ningún archivo seleccionado';
            const botonArchivo = this.previousElementSibling;
            
            if (this.files[0]) {
                botonArchivo.innerHTML = `<span>${fileName}</span>`;
                botonArchivo.style.backgroundColor = 'rgba(211, 4, 197, 0.7)';
            } else {
                botonArchivo.innerHTML = `<span>⊱⋆⊰ Adjuntar archivo ⊱⋆⊰</span>`;
                botonArchivo.style.backgroundColor = 'rgba(163, 4, 250, 0.7)';
            }
        });